    ...
    ExtendedResolver
{
    ...
    function setText(
        ...
        bytes32 ownedNode = getOwnedENSNode(node, msg.sender);
    ...
    function setAddr(
        ...
        versionable_addresses[recordVersions[ownedNode]][ownedNode][
            coinType
        ] = a;
    }
    ...
    function setABI(
        ...
        require(
            ((contentType - 1) & contentType) == 0,
            'contentType unsupported'
        );
        bytes32 ownedNode = getOwnedENSNode(node, msg.sender);

        versionable_abis[recordVersions[ownedNode]][ownedNode][
            contentType
        ] = data;
    ...
    function setInterface(
        ...
        bytes32 ownedNode = getOwnedENSNode(node, msg.sender);
        versionable_interfaces[recordVersions[ownedNode]][ownedNode][
            interfaceID
        ] = implementer;
    ...
    function setPubkey(bytes32 node, bytes32 x, bytes32 y) external override {
        bytes32 ownedNode = getOwnedENSNode(node, msg.sender);
        versionable_pubkeys[recordVersions[ownedNode]][ownedNode] = PublicKey(
            x,
            y
        );
