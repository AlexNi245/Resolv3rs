...
export const RecordsTab = ({
  ...
  const [resolverEntry, setResolv3erValue] = useState('')
  ...
  useEffect(() => {
    const foo = async () => {
      ...
      try {
        const val = await resolver!.getText('MY_NEW_TEXT_RECORD')
        setResolv3erValue(val)
      } catch (e) {
        console.log(e)
      }
    }
  ...
  return (
    <TabWrapper $isCached={isCached} data-testid="records-tab">
      <AllRecords>
        ...
        <div>Resolv3r Record : {resolverEntry}</div>
      </AllRecords>
          ...
          <div>
            {canEditRecordsWhenWrapped && !hasGlobalError ? (
              ...
              <DisabledButtonWithTooltip
                ...
                buttonText={t('details.tab  s.records.editRecords')}
                mobileWidth={150}
